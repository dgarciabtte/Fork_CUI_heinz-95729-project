...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_def", type=str, default="/root/yolov3_radar-explore/config/yolov3_cv_multi.cfg", help="path to model definition file")
    parser.add_argument("--data_config", type=str, default="/root/yolov3_radar-explore/config/0601.data", help="path to data config file")
    parser.add_argument("--pretrained_weights", type=str, default="/root/yolov3_cv_multi_0601_cv_model.pth",help="if specified starts from checkpoint model")
    parser.add_argument("--checkpoints_dir", type=str, default="/root/yolov3_radar-explore/checkpoints", help="path to save weight file")
    ...
    parser.add_argument("--backward-passes-per-step",type=float,default=1,help="number of backward iteration steps executed before updating parameters")
    ...
    lr_scaler = hvd.size()
    optimizer = torch.optim.Adam(model.parameters(), lr=float(model.hyperparams['learning_rate'])*lr_scaler,
            weight_decay=float(model.hyperparams['decay']))
    hvd.broadcast_parameters(model.state_dict(),root_rank = 0)
    hvd.broadcast_optimizer_state(optimizer,root_rank=0)
    optimizer = hvd.DistributedOptimizer(optimizer, named_parameters=model.named_parameters(),
           op= hvd.Average,backward_passes_per_step=2)
    metrics = [
        "grid_size",
        "loss",
        "x",
